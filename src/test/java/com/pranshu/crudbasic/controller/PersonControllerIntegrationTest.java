...
class PersonControllerIntegrationTest {
    ...
    void getPersonDetails_ShouldReturnPersonById() {
        ...
        Person response = restTemplate.getForObject(baseUrl + "/" + person.getPersonID(), Person.class);
        assertThat(response).isNotNull();
        assertThat(response.getPersonName()).isEqualTo(person.getPersonName());
    }
    ...
    void createPerson_ShouldAddNewPerson() {
        ...
        assertThat(allPersons).extracting(Person::getPersonName).contains("C");
    }

    @Test
    void deletePerson_ShouldRemovePerson() {
        Person person = personRepository.findAll().get(0);
        restTemplate.delete(baseUrl + "/" + person.getPersonID());
        List<Person> remainingPersons = personRepository.findAll();
        assertThat(remainingPersons).hasSize(1);
        assertThat(remainingPersons).extracting(Person::getPersonName).doesNotContain(person.getPersonName());
    }
}
